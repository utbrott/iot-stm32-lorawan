...
static bool VerifyTxFreq( uint32_t freq )
{
    ...
    if( Radio.CheckRfFrequency( freq ) == false )
    {
      ...
      {
        DBG_PRINTF("\tChannel ** CANNOT BE ADDED ** in Japan: Freq %d Hz is outside channels 24~38\n\r", freq);
        return false; // Japan region channels used are 24~38 (Center Freq: 920.6 MHz to 923.4 MHz @ 200 kHz max BW each)
    ...
    if( nbEnabledChannels > 0 )
    {
        for( uint8_t  i = 0, j = randr( 0, nbEnabledChannels - 1 ); i < AS923_MAX_NB_CHANNELS; i++ )
        {
            ...
            while( TimerGetElapsedTime( carrierSenseTime ) < AS923_CARRIER_SENSE_TIME )
            {
              ...
              #else
                rssi = Radio.GetChannelRssi( MODEM_LORA, Channels[channelNext].Frequency); 
              #endif 
